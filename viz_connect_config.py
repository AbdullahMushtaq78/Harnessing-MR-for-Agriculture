	...
	if vizconnect.isPendingInit('event', _name, initFlag, initList):
		...
		if initFlag&vizconnect.INIT_MAPPINGS:
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				def update(event):
					if rawInput['left_touch_controller'].isButtonDown(1):# make=Oculus VR, model=Touch Controller, name=left_touch_controller, signal=Button Y
	...
	if vizconnect.isPendingInit('event', _name, initFlag, initList):
		...
		if initFlag&vizconnect.INIT_MAPPINGS:
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				#VC: set the update function which checks for input signals
				def update(event):
					if rawInput['left_touch_controller'].isButtonDown(0):# make=Oculus VR, model=Touch Controller, name=left_touch_controller, signal=Button X
						event.sendOnce(e=viz.Event(mag=1))
					if rawInput['left_touch_controller'].isButtonDown(0):# make=Oculus VR, model=Touch Controller, name=left_touch_controller, signal=Button X
						event.sendEvent(e=viz.Event(mag=1))
				rawEvent[_name].setUpdateFunction(update)
	...
	if vizconnect.isPendingInit('event', _name, initFlag, initList):
		...
		if initFlag&vizconnect.INIT_MAPPINGS:
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				#VC: set the update function which checks for input signals
				def update(event):
					if rawInput['right_touch_controller'].isButtonDown(0):# make=Oculus VR, model=Touch Controller, name=right_touch_controller, signal=Button A
						event.sendOnce(e=viz.Event(mag=1))
					if rawInput['right_touch_controller'].isButtonDown(0):# make=Oculus VR, model=Touch Controller, name=right_touch_controller, signal=Button A
						event.sendEvent(e=viz.Event(mag=1))
				rawEvent[_name].setUpdateFunction(update)
